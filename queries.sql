...
SELECT * FROM `tienda`.`producto` WHERE `producto`.`precio` = (SELECT MAX(`producto`.`precio`) FROM `tienda`.`producto` WHERE `producto`.`codigo_fabricante` = (SELECT `codigo` FROM `tienda`.`fabricante` WHERE `fabricante`.`nombre` = 'Lenovo'));
